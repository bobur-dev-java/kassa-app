...
public record MoneyTransactionSpecification(MoneyTransactionFilter filter) implements Specification<MoneyTransaction> {


    @Nullable
    @Override
    public Predicate toPredicate(Root<MoneyTransaction> root,
        ...
        if (filter.getMoneyType() != null) {
            predicates.add(cb.equal(root.get("moneyType"), filter.getMoneyType()));
        }
